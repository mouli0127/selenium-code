...
/**
 * -> 1.Read the excel using apache poi 2.scan every row 3.read every row info
 * and keep inside the action object 4.keep every action obj to the list 5.pass
 * the list of action objects to the test class
 */
public class KeyDrivenExcelUtils {
	...
	public Action getAction(XSSFSheet worksheet, int columns, int i) {
		...
		Action action = new Action(steps.get(0), steps.get(1), steps.get(2), steps.get(3), steps.get(4), steps.get(5),
				steps.get(6));
		return action;
	}
